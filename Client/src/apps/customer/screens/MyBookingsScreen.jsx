...
/* ── Status config ───────────────────────────────────── */
const STATUS = {
  confirmed:     { label: 'Upcoming',     bg: '#EFF6FF', color: '#1D4ED8', dot: '#3B82F6', bar: '#3B82F6' },
...
const BookingCard = ({ booking, onClick }) => {
  ...
  return (
    <div
      onClick={onClick}
      className="group relative bg-white border border-slate-100 rounded-[20px] mb-0 cursor-pointer overflow-hidden transition-all duration-300 hover:border-emerald-200 hover:shadow-xl hover:shadow-emerald-900/6 active:scale-[0.985]"
    ...
    >
      ...
      <div className="relative h-36 overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4">
          ...
          <p className="text-white/50 text-[10px] font-bold uppercase tracking-widest">
            ID: {booking.id?.slice(-8) || booking.id}
          </p>
        </div>
      ...
      <div className="p-5">
        ...
        {/* In-progress technician banner */}
        {isInProgress && booking.technicianName && (
          <div className="mb-4 p-3.5 rounded-2xl bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-100 flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-emerald-500 flex items-center justify-center shrink-0 shadow-md shadow-emerald-200">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-black text-emerald-900 truncate">
                {booking.technicianName} · On the way
              </p>
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-3.5 mb-5">
          ...
          {booking.technicianName && !isInProgress && (
        ...
        <div className="flex gap-2">
          {isCompleted ? (
            <>
              <button className="flex-1 py-2.5 rounded-xl border border-slate-200 bg-white text-xs font-bold text-slate-600 hover:bg-slate-50 transition-colors active:scale-95">
                Invoice
              </button>
              <button className="flex-1 py-2.5 rounded-xl bg-emerald-50 border border-emerald-100 text-xs font-bold text-emerald-700 hover:bg-emerald-100 transition-colors active:scale-95">
                Rate Service
              </button>
            </>
          ) : isCancelled ? (
            <button
              ...
              className="w-full py-3 rounded-xl bg-slate-100 text-slate-500 text-xs font-bold"
            >
              Book Again
            </button>
          ) : (
            <button
              ...
              className="w-full py-3 rounded-xl bg-slate-900 text-white text-xs font-bold shadow-md shadow-slate-200 hover:bg-slate-800 active:scale-95 transition-all"
            >
              Manage Booking
            </button>
          )}
...
/* ── Skeleton loader ────────────────────────────────── */
const SkeletonCard = () => (
  <div className="bg-white border border-slate-100 rounded-[20px] mb-0 overflow-hidden animate-pulse" style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
  ...
  return (
    <section style={{ background:'#f9fafb' }} className="min-h-screen py-[40px] px-4 md:px-8">
      ...
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="bk-fade mb-10 flex justify-between items-start">
          <div>
            ...
            <p className="text-sm font-medium text-slate-500">
              Track and manage all your appliance repair bookings
            </p>
          </div>
          <button
            onClick={() => refetch()}
        ...
        {!isLoading && !error && (
          <div
            className="bk-fade rounded-[20px] px-8 py-8 grid grid-cols-2 md:grid-cols-4 gap-4 mb-10"
        ...
        {error && (
          <div className="py-20 text-center flex flex-col items-center bk-fade">
            ...
            <button
              onClick={() => refetch()}
              className="px-6 py-2.5 bg-amber-500 text-white text-sm font-bold rounded-full shadow-md shadow-amber-100 active:scale-95"
            >
              Try again
            </button>
          </div>
